<?php
  ...
  if ($sess_userpass=$cfg_picpass) $pass==1;
  ...
  if ($picanz<1 && $str) {
   $query="SELECT COUNT(*) FROM $mysql_tbpic AS tab1 $favquery1 WHERE ((tab1.".$column1['kategorie']." LIKE '$cat' ||  tab1.".$column1['suchbegriffe']." LIKE '%$krit;%') && tab1.".$column1['status']."=2 && tab1.".$column1['opt4']."='$pass' $favquery2)";
  ...
  // Get Image(s)
  $query="SELECT *$rank FROM $mysql_tbpic AS tab1 $favquery1 WHERE ((tab1.".$column1['kategorie']." LIKE '$cat' ||  tab1.".$column1['suchbegriffe']." LIKE '%$krit;%') && tab1.".$column1['status']."=2 && tab1.".$column1['opt4']."='$pass' $rank2 $favquery2) ORDER BY $order ASC LIMIT ".($str-1).",$req";
  $res = mysql_query($query,$mysql_nr[$connr]) or myerror($query,__LINE__,__FILE__,mysql_error($mysql_nr[$connr]));
  $picsget=0;
  while ($row = mysql_fetch_array($res)) {
   $size = @GetImageSize ($cfg_picpath."/".stripslashes($row[$column1['bild']]));
   $picwidth= round($cfg_aufl_fakt[$aufl] * $size[0]);
   $picheight= round($cfg_aufl_fakt[$aufl] *  $size[1]);
   $pic[$picsget]="<img src=\"".$cfg_picpath."/".stripslashes($row[$column1['bild']])."\" border=0 width=$picwidth height=$picheight galleryimg=no class=picborder>";
...
//Get Arrows
  if (!$diashow) {
   ...
   if (($picanz-$str)>=0 && (($str-$req)>=0 && $str>1) || ($str-$req)>0) $toe_arrowle="<td><a href=site.php?cat=$cat&catname=$catname&fav=$fav&krit=$krit&ranked=$ranked&str=1&req=$req&picanz=$picanz&aufl=$aufl&PHPSESSID=$PHPSESSID title=\"erstes Bild\" class=menulink><img src=\"../img/arrow-leftend$arrowsub.gif\" alt=\"erstes Bild\" border=0></a></td>";
   if (($picanz-$str)>=0 && ($str-$req)>=$req) $toe_arrowl="<td><a href=site.php?cat=$cat&catname=$catname&fav=$fav&krit=$krit&ranked=$ranked&str=".($str-$req)."&req=$req&picanz=$picanz&aufl=$aufl&PHPSESSID=$PHPSESSID title=\"vorherriges Bild\" class=menulink><img src=\"../img/arrow-left$arrowsub.gif\" alt=\"vorherriges Bild\" border=0></a></td>";
   if (($str+$req)<=$picanz-$req) $toe_arrowr="<td><a href=site.php?cat=$cat&catname=$catname&fav=$fav&krit=$krit&ranked=$ranked&str=".($str+$req)."&req=$req&picanz=$picanz&aufl=$aufl&PHPSESSID=$PHPSESSID title=\"nächstes Bild\" class=menulink><img src=\"../img/arrow-right$arrowsub.gif\" alt=\"nächstes Bild\" border=0></a></td>";
   if (($str+$req)<=$picanz) $toe_arrowre="<td><a href=site.php?cat=$cat&catname=$catname&fav=$fav&krit=$krit&ranked=$ranked&str=".((floor($picanz / $req) * $req))."&req=$req&picanz=$picanz&aufl=$aufl&PHPSESSID=$PHPSESSID title=\"letztes Bild\" class=menulink><img src=\"../img/arrow-rightend$arrowsub.gif\" alt=\"letztes Bild\" border=0></a></td>";
  }
 ...
 statistic($cfg_sitename, str_replace("&nbsp;-&gt;&nbsp;","/",rawurldecode($catname)), $cook_id, "visit", $connr);
 ...
 <body <?php echo"bgcolor=$bgc_main";?> class=mainbody>
  <form>
   <table width=100% cellspacing=0 cellpadding=0 border=0>
    <tr height=30><td align=right class=headln><?php echo stripslashes(rawurldecode($catname)); ?></td></tr>
